...
void Camera::move() {
    ...
    if (GetAsyncKeyState(   'A'  )) pos_ -= Vector {  dir_.z_, dir_.y_, -dir_.x_ } * speed_;
    if (GetAsyncKeyState(   'D'  )) pos_ -= Vector { -dir_.z_, dir_.y_,  dir_.x_ } * speed_;
    if (GetAsyncKeyState(   'W'  )) pos_ += dir_ * speed_;
    if (GetAsyncKeyState(   'S'  )) pos_ -= dir_ * speed_;
    if (GetAsyncKeyState(VK_SPACE)) pos_ -= Vector { 0,  1, 0 } * speed_;
    if (GetAsyncKeyState(VK_SHIFT)) pos_ -= Vector { 0, -1, 0 } * speed_;
